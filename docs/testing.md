...
inputs. `prodRelease` requires `app/src/prod/google-services.json` with `com.reals.app` and complete production
release signing inputs. Production defaults to `https://reals-api.duckdns.org/` and can still be overridden with
