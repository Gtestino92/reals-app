...
- `PUT /api/me/profile/photos/reorder`: reorder authenticated user's existing profile photos. Body: `placements` array with every current photo exactly once and final positions from 1 to 9. Holes are allowed. Response: `PhotoResponse[]`, same shape as `GET /api/me/profile/photos`, sorted by final position. This only changes `position`, does not reupload files, does not re-run validation or moderation, and does not move an active profile back to draft.
