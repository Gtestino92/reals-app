...
- `GET /api/matches/{matchId}/chat`: fetch active first chat for match. Includes `partner`, `myDecision`, `partnerDecision`, `expiresAt`, `inactivityExpiresAt`, required `serverTime` captured while building the response and nullable `guidance` metadata. New first chats initialize guidance; legacy rows may return `guidance = null`.
